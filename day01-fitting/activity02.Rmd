...
4. Does the relationship look linear?
  ...
  *The relationship is a linear relationship because the points  follow a straight line pattern *
...
pf_score = intercept_value + slope_value * 0
...
*The value of $R^2$ is approximately 0.6342.*
...
*It means that around 63.42% of the variability in the dependent variable is explained by the independent variables included in the model*
...
```{r train-fit-lm}
slr_train <- lm_spec %>% 
  fit(pf_score ~ pf_expression_control, data )
...
# Create an initial 80-20 split of the diamonds dataset
...
# Fit the simple linear regression model using the training dataset
slr_train <- lm_spec %>% 
...
10. Interpret each of the estimated parameters from (10) in the context of this research question.
  That is, what do these values represent?
*Intercept-: The intercept is not practically interpretable in this context since it represents the predicted pf_score when carat is zero, which is not meaningful for diamonds.*
*Slope for the carat variable (7736.950)-:The slope represents the estimated increase in pf_score for each additional carat in a diamond. Specifically, for each added carat, the predicted pf_score is expected to increase by approximately 7736.950 points.*
...
```{r glance-test}
test_aug <- augment(slr_train, new_data = diamonds_test)
test_aug
...
*If the R^2 value from the testing data is similar to the training data (R^2≈0.6342), it suggests that the model generalizes well, and there is no significant overfitting.*

*If the R^2 value from the testing data is significantly lower than the training data, it may indicate that the model did not generalize well to new data, and there might be issues such as overfitting.*
...
```{r augment}
# Assuming slr_train is the correct object used to fit the linear regression model
train_aug <- augment(slr_train, new_data = diamonds_train)

# Display the augmented data frame
train_aug
...
12. Based on the histogram, does the nearly normal residuals condition appear to be violated?
  Why or why not?
*NO, the histogram is roughly symmetric and bell-shaped, and this suggests that the nearly normal residuals condition is met*
...
*The spread of residuals appears to be fairly consistent as the fitted values change, it suggests that the constant variability condition is met.*
